...
app.patch("/settings", async (req, res) => {
  ...
  if (signedInUserId === targetUserId) {
    return res.status(400).json({
      message: "user cant change his roles",
    });
  }
  if (signedInUser?.address_book_role !== "admin") {
    return res.status(400).json({
      message: "employee and hr cant change user roles",
    });
  }
  if (targetUser?.addressBookRole === addressBookRole) {
    return res.status(400).json({
      message: "role is already chosen",
    });
  }
...
app.put("/edit/:id", async (req, res) => {
  ...
  if (!updatedEmployee || typeof updatedEmployee !== "object") {
    return res.status(400).json({
      message: " updatedEmployee object is required",
    });
  }
